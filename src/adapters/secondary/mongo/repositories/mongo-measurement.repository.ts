  ...
  implements IMeasurementRepository
{
  ...
  async getLastMeasurements(id: string, from: Date, session?: ClientSession): Promise<Array<Measurement>> {
    return await this.find(
      { station: this.normalizedFilter({ id }), datetime: { $gte: from } } as any as Partial<Measurement>,
      session,
    );
  }
}
